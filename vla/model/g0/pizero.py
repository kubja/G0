...
class PiZero(nn.Module):
    ...
    def freeze_by_stage(self, stage: str):
        if stage in {"full-finetune", "vla-full-train"}:
            logger.info(f"[TRAINABLE]        🔥   =>> Vision Backbone `{self.vision_tower}`", ctx_level=1)  # noqa: E501
            logger.info(f"[TRAINABLE]        🔥   =>> VLM expert `{self.joint_model.mixtures['vlm']}`", ctx_level=1)  # noqa: E501
            logger.info(f"[TRAINABLE]        🔥   =>> Action expert `{self.joint_model.mixtures['action']}`", ctx_level=1)  # noqa: E501
            pass
        elif stage in {"action-expert-only"}:
            self.freeze_non_lora_weights_in_vlm()
            logger.info(f"[FROZEN]           🥶   =>> Vision Backbone `{self.vision_tower}`", ctx_level=1)  # noqa: E501
            logger.info(f"[FROZEN]           🥶   =>> VLM expert `{self.joint_model.mixtures['vlm']}`", ctx_level=1)  # noqa: E501
            logger.info(f"[TRAINABLE]        🔥   =>> Action expert `{self.joint_model.mixtures['action']}`", ctx_level=1)  # noqa: E501
        else:
